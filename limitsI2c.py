...
class limitsI2c:
    ...
    def __init__(self):
        ...
        left_i2c_bus = (busio.I2C(board.SCL, board.SDA))
        right_i2c_bus = (busio.I2C(board.SCL, board.SDA))
        self.left_mcp = MCP23017(left_i2c_bus, address=0x21)
        self.right_mcp = MCP23017(right_i2c_bus, address=0x20)
        ...
        self.right_joints = {5: "ankle_pitch",
                             4: "knee",
                             3: "ankle_roll",
                             1: "hip_pitch",
                             2: "hip_roll",
        ...
        self.right_last_state = {"knee": True,
                                 "ankle_pitch": True,
                                 "ankle_roll": True,
                                 "hip_pitch": True,
                                 "hip_roll": True,
        ...
        self.left_last_state = {"knee": True,
                                "ankle_pitch": True,
                                "ankle_roll": True,
                                "hip_pitch": True,
                                "hip_roll": True,
        ...
        self.limits = {"left_knee": True,
                       "left_ankle_pitch": True,
                       "left_ankle_roll": True,
                       "left_hip_pitch": True,
                       "left_hip_roll": True,
                       "left_hip_yaw": True,
                       "right_knee": True,
                       "right_ankle_pitch": True,
                       "right_ankle_roll": True,
                       "right_hip_pitch": True,
                       "right_hip_roll": True,
                       "right_hip_yaw": True}
    ...
    def mcp_bringup(self):  # configure the pins on the board as INPUT PULLUP
        for left_pin in range(0, 6):
            # defines the mcp pins (A0 is 0, A1 is 1 and so on)
            self.left_buttons.append(self.left_mcp.get_pin(left_pin))
    ...
    def read_limits(self):
        while True:
            ...
            for left_button in self.left_buttons:
                ...
                # reads the last state of the inspected pin from the last state dictionary
                last_state = self.left_last_state.get(joint)

                if (current_value != last_state):
                    # updates the last state (true/false) of the pin in the dictionary
                    self.left_last_state[joint] = current_value
            ...
            for right_button in self.right_buttons:
                ...
                # reads the last state of the inspected pin from the last state dictionary
                last_state = self.right_last_state.get(joint)

                if (current_value != last_state):
                    # updates the last state (true/false) of the pin in the dictionary
                    self.right_last_state[joint] = current_value
